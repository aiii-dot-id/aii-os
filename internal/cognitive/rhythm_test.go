...
func TestRhythmCapacityGating(t *testing.T) {
	...
	// .
	r.OnAlarm(context.Background(), "rhythm", "wall", 0, "")
	if selfModel.runs != 0 || review.runs != 0 {
		t.Fatalf("reflection must not run on the wall pass: self_model=%d review=%d", selfModel.runs, review.runs)
	}
	// .
	r.OnAlarm(context.Background(), ReflectSelfModelAlarm, "life", 50, "")
	r.OnAlarm(context.Background(), ReflectIdentityReviewAlarm, "life", 100, "")
	if selfModel.runs != 1 || review.runs != 1 {
		t.Fatalf("reflection runs on its life alarm: self_model=%d review=%d", selfModel.runs, review.runs)
	}
}
...
func TestADeferredPassIsNotASkippedPass(t *testing.T) {
	...
	dream, consolidate := &fakeOwner{}, &fakeOwner{}
	gate := heldGate()
	r := NewRhythm(&fakeRaw{n: 3}, gate, dream, consolidate, &fakeOwner{}, &fakeOwner{})
	res := r.OnAlarm(context.Background(), "rhythm", "wall", 0, "")
	if res.Accepted || dream.runs+consolidate.runs != 0 || !r.lastConsolidate.IsZero() {
		t.Fatalf("a deferred pass must run nothing and advance nothing: %+v dream=%d consolidate=%d", res, dream.runs, consolidate.runs)
	}
	// .
	gate.held = false
	res = r.OnAlarm(context.Background(), "rhythm", "wall", 0, "")
	...
	if dream.runs+consolidate.runs != 1 {
		t.Fatalf("the deferred work did not happen on the next pass: dream=%d consolidate=%d", dream.runs, consolidate.runs)
	}
	if gate.ends != 1 {
