...
func TestVerifyRejectsDecompressedBytesAfterEndBlocks(t *testing.T) {
	...
	bomb := append(tarBytes, make([]byte, 4<<20)...)
...
func TestVerifyRejectsForbiddenPathComponents(t *testing.T) {
	...
	for _, path := range []string{
		root + "/install-root/nul.txt",
		root + "/install-root/com1",
		root + "/install-root/trailing.",
		root + "/install-root/sp ace.txt",
		root + "/install-root/ütf8.txt",
		root + "/install-root/" + strings.Repeat("c", 256),
	} {
