...
// .
// .
func TestEchoGuestMemoryStaysFlatAcrossInvokes(t *testing.T) {
	m := mustLoad(t, "component-echo.wasm", Config{})
	frame := []byte(`{"jsonrpc":"2.0","id":"1","method":"plugin.invoke","params":{"operation":"echo"}}`)
	if _, err := m.Invoke(context.Background(), frame); err != nil {
		t.Fatal(err)
	}
	pages := m.mod.Memory().Size()
	for i := 0; i < 5000; i++ {
		if _, err := m.Invoke(context.Background(), frame); err != nil {
			t.Fatalf("invoke %d: %v", i, err)
		}
	}
	if got := m.mod.Memory().Size(); got != pages {
		t.Fatalf("guest memory grew from %d to %d bytes over 5000 calls — post-return did not reclaim the call's allocations", pages, got)
	}
}
...
func TestStubDenialShape(t *testing.T) {
	...
	if denial.Message == "" {
		t.Error("denial must name the requirement in message")
	}
	// .
