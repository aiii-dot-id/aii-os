...
func TestNormalizeIsExactlyTheRulesInTheContract(t *testing.T) {
	for _, tc := range []struct {
		...
		{"192.0.2.2", "one nine two zero two two"},
		{"AII OS", "aii os"},
...
func TestTermRecallSeesWhatWERAveragesAway(t *testing.T) {
	refs := [][]string{
		...
		Normalize("ivy entered safe mode"),
	}
	hyps := [][]string{
		Normalize("the ledger is frozen"),
		Normalize("ian entered safe mode"),
	}
	scores := ScoreTerms([]string{"ivy", "ledger", "safe mode"}, refs, hyps)

	byTerm := map[string]TermScore{}
	for _, s := range scores {
		byTerm[s.Term] = s
	}
	if got := byTerm["ivy"]; got.InRef != 1 || got.InHyp != 0 || got.Recall != 0 {
		t.Fatalf("ivy: %+v — the identity's own name was lost and recall did not say so", got)
	}
