...
func TestRepeatedAndUnknownResultsAreRefused(t *testing.T) {
	...
	r := Aggregate(m, []ClipResult{
		...
		ran("elsewhere.wav", "invented clip", time.Second, 0),
	})
...
func TestOneUnheardTermFailsTheContractEvenWhenTheAverageIsFine(t *testing.T) {
	said := "ivy checked the ledger and the outbox during quiesce"
	m := testManifest([]string{"ivy", "ledger", "outbox", "quiesce"},
	...
	if !strings.Contains(joined, "DOMAIN VOCABULARY") || !strings.Contains(joined, "ivy") {
...
func TestViolationsReportsEveryMissAtOnce(t *testing.T) {
	m := testManifest([]string{"ivy"},
		speechClip("a.wav", "quiet", "ivy is here"),
		silenceClip("s.wav"),
