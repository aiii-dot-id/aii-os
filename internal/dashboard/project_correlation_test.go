...
func TestProjectActionsEchoTheRequestID(t *testing.T) {
	h := &WSHandler{
		ProjectAct: func(req ProjectRequest) error {
			if req.Action == "create" && req.Name == "refuse-me" {
	...
	if msg.RequestID != "42" {
		t.Fatalf("the success answer does not wear the request id: %q", msg.RequestID)
	}
	...
	if msg.RequestID != "43" {
		t.Fatalf("the refusal does not wear the request id: %q", msg.RequestID)
	}
