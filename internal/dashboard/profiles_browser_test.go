...
run(() => {
  S.config = { plugins: { autoload: 'T1', installed: [], catalog: [], catalog_url: '',
    auth_profiles: [
      ...
      { name: 'google-cal', scheme: 'oauth2', provider: 'google', client_id: 'g', scopes: ['https://www.googleapis.com/auth/calendar.readonly'], hosts: ['www.googleapis.com:443'], state: 'connected', expires_at: '2026-09-12T21:00:00Z', device: { user_code: 'WXYZ-1234', verification_uri: 'https://auth.example/device', expires: '2026-09-12T20:30:00Z' } },
  ...
  st.querySelector('[data-profile-paste-input="gh"]').value = 'http://127.0.0.1:8187/oauth/callback?code=c&state=s';
  st.querySelector('[data-profile-complete="gh"]').click();
  assert(frames.some(f => f.type === 'profile_signin_complete' && f.profile === 'gh' && f.input.includes('code=c')), 'the pasted redirect completes the consent');
  ...
  assert(S.connectRequest === null, 'the request is consumed');
	...
	for _, path := range []string{"static/views/plugins.js"} {
	...
	modules["/views/settings.js"] = []byte(`export const saved = []; export function saveConfigSection(section) { saved.push(section); } export const sent = []; export function sendConfigChanges(section, ch) { saved.push(section); sent.push(ch); } export function configFeedbackHTML() { return ''; }`)
