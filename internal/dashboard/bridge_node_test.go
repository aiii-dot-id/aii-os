...
let t = topicOf({ type: 'status', stats: { name: 'Nova' } });
assert(t && t.topic === 'status' && t.data.name === 'Nova', 'status projection');
...
bridge.publish('status', { name: 'Nova' });
await settle();
assert(seen && seen.name === 'Nova', 'declared+subscribed topic relays');
