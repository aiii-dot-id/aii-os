...
func TestSetupAppliesAndSwaps(t *testing.T) {
	...
	restricted, err := fileperm.IsRestrictedToOwner(filepath.Join(dir, "config.json"))
	if err != nil {
		t.Fatal(err)
	}
	if !restricted {
		t.Fatal("config.json may carry credentials and is readable beyond its owner")
	}
