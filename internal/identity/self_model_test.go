...
// .
// .
func TestAPortraitIsAnEdgeEndpoint(t *testing.T) {
	engine, st, lg, kp, _ := setupEngine(t)
	refs := selfModelSources(t, st, lg, kp)
	if _, err := engine.ExecuteAction(context.Background(), "verb", "commit", selfModelArgs("syn_one", refs)); err != nil {
		t.Fatal(err)
	...
	// .
	second := selfModelArgs("syn_two", refs)
	second["synthesis_text"] = "I am learning to trust evidence, and to name who taught me."
	second["previous_synthesis_id"] = "syn_one"
	if _, err := engine.ExecuteAction(context.Background(), "verb", "commit", second); err != nil {
		t.Fatal(err)
	}
	if _, err := engine.ExecuteAction(context.Background(), "verb", "commit", map[string]interface{}{
		"variant": "edge.create", "edge_type": "SHAPED_BY", "from_id": "syn_one", "to_id": "b_self",
	}); err != nil {
		t.Fatalf("a belief could not cite the portrait it came from: %v", err)
	}
	// .
	// .
	if standing, err := st.StandingFor("b_self"); err != nil || standing != "new" {
		t.Fatalf("standing after citing a portrait = %q, %v; want new", standing, err)
	}
	// .
	// .
	exists, err := st.EntityExists("syn_two")
	if err != nil || !exists {
		t.Fatalf("the current portrait is not an entity either: %v %v", exists, err)
	}
}
