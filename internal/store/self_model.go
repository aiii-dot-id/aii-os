...
// .
// .
func scanSelfModelSyntheses(rows *sql.Rows) ([]SelfModelSynthesis, error) {
...
func (s *Store) PromptIdentity() (PromptIdentity, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	charter, operatorName, hasOperatorRelationship, err := s.operatorCharterLocked()
	if err != nil {
	...
	priorities, err := s.activePrioritiesLocked()
	if err != nil {
		return PromptIdentity{}, err
	}
